...
func getRsaKey() string {
	...
	return preGeneratedRsaKeys[index]
}

func getPreGeneratedKeys() *PreGeneratedKeys {
	return &PreGeneratedKeys{EncryptionKey: getRsaKey(), SigningKey: getRsaKey()}
}

func getInitializeOptions(dbName string, clean bool) (*SdkInitializeOptions, error) {
	credentials, err := test_utils.GetCredentials()
	if err != nil {
		return nil, tracerr.Wrap(err)
	}

	databasePath, err := test_utils.GetDBPath(dbName)
	if err != nil {
		return nil, tracerr.Wrap(err)
	}
	...
	return &SdkInitializeOptions{
		ApiURL:                    credentials.ApiUrl,
		AppId:                     credentials.AppId,
		KeySize:                   1024,
		DatabasePath:              databasePath,
		EncryptionSessionCacheTTL: int64(24 * time.Hour),
		LogNoColor:                false,
		LogLevel:                  -1,
		InstanceName:              fmt.Sprintf("Inst_name_%s", dbName[len(dbName)-15:]),
		DatabaseEncryptionKey:     encryptionKey,
		Platform:                  "mobilesdk-tests",
	}, nil
}

func getTestAccount(dbName string, clean bool) (*MobileSDK, *AccountInfo, error) {
	initOpts, err := getInitializeOptions(dbName, clean)
	if err != nil {
		return nil, nil, tracerr.Wrap(err)
	}

	mSDK, err := Initialize(initOpts)
	if err != nil {
		return nil, nil, tracerr.Wrap(err)
	}
	...
	accountInfo, err := mSDK.CreateAccount(&CreateAccountOptions{
		DisplayName:      fmt.Sprintf("Disp_name_%s", dbName[len(dbName)-15:]),
		DeviceName:       fmt.Sprintf("Dev_name_%s", dbName[len(dbName)-15:]),
		SignupJWT:        jwt,
		ExpireAfter:      5 * 365 * 24 * 60 * 60,
		PreGeneratedKeys: getPreGeneratedKeys(),
	})
	if err != nil {
		return nil, nil, tracerr.Wrap(err)
	}

	return mSDK, accountInfo, nil
}
