...
- @all: \[new feature\] Anonymous SDK now can create `AnonymousEncryptionSession`. This session exposes six functions: `EncryptMessage`, `DecryptMessage`, `EncryptFile`, `DecryptFile`, `EncryptFileFromPath`, `DecryptFileFromPath`.
...
- @all: \[new feature\] Add functions `EncryptionSession.ListRecipients` to list all recipients of an `EncryptionSession`
...
- @all: \[**breaking change**\] `session.RevokeRecipients` can now revoke SymEncKeys & TMR Accesses, and takes the corresponding arguments.
